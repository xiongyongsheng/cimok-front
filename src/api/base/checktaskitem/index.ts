...
// 导入校验任务的具体项目 Excel
export function importChecktaskItem(data) {
  return defHttp.post({ url: '/base/checktask-item/import-excel', data });
}
export function getTempltateChecktaskItem() {
  return defHttp.download({ url: '/base/checktask-item/get-import-template' });
}

// 导出校验任务的具体项目 Excel
export function exportChecktaskItem(params) {
  return defHttp.download(
    { url: '/base/checktask-item/export-excel', params },
    '校验任务的具体项目.xls'
  );
}
