<script lang="ts" setup>
  ...
  import { deleteMapdata, exportMapdata, getMapdataPage } from '@/api/wafermap/mapdata'
  ...
  <div>
    ...
    <BasicTable @register="registerTable">
      <template #toolbar>
        ...
        <a-button v-auth="['wafermap:mapdata:export']" :preIcon="IconEnum.EXPORT" @click="handleExport">
          {{ t('action.export') }}
        </a-button>
      </template>
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'action'">
          <TableAction
                  :actions="[
                          { icon: IconEnum.EDIT, label: t('action.edit'), auth: 'wafermap:mapdata:update', onClick: handleEdit.bind(null, record) },
                          {
                          icon: IconEnum.DELETE,
                          danger: true,
                          label: t('action.delete'),
                          auth: 'wafermap:mapdata:delete',
                          popConfirm: {
