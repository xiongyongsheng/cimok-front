 ...
 * @LastEditTime: 2024-08-25 13:30:18
 * @LastEditors: 卢靖康
-->
...
import { exportWafer, getWaferPage } from "@/api/wafermap/wafer";
    ...
    <BasicTable @register="registerTable">
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'waferId'">
          <router-link :to="`/wafermap/search/wafer/detail?id=${record.id}`">
            {{ record.waferId }}
          </router-link>
        </template>
            ...
            :actions="[
              {
                icon: IconEnum.EDIT,
                label: '编辑',
                auth: 'wafermap:wafer:update',
                onClick: () => {
                  router.push(`/wafermap/search/wafer/detail?id=${record.id}&edit=true`);
                },
              },
              // {
